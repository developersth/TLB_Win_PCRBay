    ...
    public partial class frmCRBay : Form
    {
        ...
        private void InitialDatagrid()
        {
            string strSQL = "select t.bay_no,t.card_reader_name" +
                            ...
                            " from tas.VIEW_CARDREADER_INITAIL t " +
                            " where t.ISLAND_NO ='" +mBayNo +"'" +
                            " and t.Is_Enabled ='1'" +
            ...
            try
            {
                if (OraDb.OpenDyns(strSQL, "TableName", ref vDataset))
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if (Convert.ToInt16(dt.Rows[i]["control_comport"]) == 0)
                        {
                            mCRBay[i].BayNo = dt.Rows[i]["bay_no"].ToString(); mCRBay[i].ComportID = Convert.ToInt32(dt.Rows[i]["comp_id"]);
                        }
                        if (Convert.ToInt16(dt.Rows[i]["control_comport1"]) == 0)
                        {
                            mCRBay[i].BayNo = dt.Rows[i]["bay_no"].ToString(); mCRBay[i].ComportID = Convert.ToInt32(dt.Rows[i]["comp_id1"]);
                        }
                    }
                }
